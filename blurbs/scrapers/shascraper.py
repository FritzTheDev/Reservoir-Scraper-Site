...
def getLastSunday():
    ...
    elif today.isoweekday() == 1 or today.isoweekday() == 2 or today.isoweekday() == 3 or today.isoweekday() == 4 or today.isoweekday() == 5 or today.isoweekday() == 6:
        return date.isoformat(date.fromordinal(today.toordinal() - today.isoweekday()))
    else:
        return "Error"
        
def scrape():
  raw_html = simple_get(f'http://cdec.water.ca.gov/dynamicapp/QueryDaily?s=SHA&end={getLastSunday()}')
  html = BeautifulSoup(raw_html, 'html.parser')
  dataNodes = html.find_all("td", align="right")
  dataStrings = list(map(lambda x: x.string, dataNodes))
  slicedNodeList = [dataStrings[-120: -106], dataStrings[-105:-91], dataStrings[-90:-76], dataStrings[-75:-61], dataStrings[-60:-46], dataStrings[-45:-31], dataStrings[-30:-15], dataStrings[-15:]]
    ...
    for index, day in enumerate(data[1:]):
        try:
            ...
            if (index == 7):
                ...
                dayBeforeInflow = float(data[(index - 1)][6].replace(',', ''))
                dayAfterInflow = float(data[(index + 1)][6].replace(',',''))
